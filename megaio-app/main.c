...
u8 dirLoad() {
    ...
    resp = bi_cmd_dir_load("/MEGA", DIR_OPT_SORTED); //load system dir
    if (resp)return resp;

    bi_cmd_dir_get_size(&dir_size); //load dir

    if (dir_size > G_SCREEN_H)dir_size = G_SCREEN_H;

    bi_cmd_dir_get_recs(0, dir_size, G_SCREEN_W); //request dir reccords


    for (i = 0; i < dir_size; i++) {

        resp = bi_rx_next_rec(&inf); //read dir reccord
        if (resp)return resp;
...
u8 fileReadToRam() {//read readme.txt to ram and print to the screen
    ...
    resp = bi_cmd_file_read(buff, G_SCREEN_W);
    if (resp)return resp;
...
u8 fileReadToRom() {//read readme.txt to rom memory and print to the screen. m68k have no acces to rom memory during io DMA
    ...
    resp = bi_cmd_file_read_mem(rom_dst, G_SCREEN_W);
    if (resp)return resp;

    resp = bi_cmd_file_close();
    if (resp)return resp;

    bi_cmd_mem_set(0, rom_dst + G_SCREEN_W, 1); //end of string
...
u8 romPath() {//read path to current rom file from system config
    ...
    resp = bi_cmd_file_open("MEGA/registery.dat", FA_READ);
    if (resp)return resp;
